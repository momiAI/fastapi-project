...
async def add_img_cottage(db : DbDep, id_cott : int, images : UploadFile):
     img_stmt = await db.images.insert_to_database(ImageAdd(name_img = str(id_cott) + str(images.filename)))
     await db.asociatimagecottage.insert_to_database(AsociationImagesCottage(id_img = img_stmt.id, id_cottage= id_cott))
